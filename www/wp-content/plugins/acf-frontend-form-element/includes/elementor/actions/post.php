...
class ActionPost extends ActionBase {
	...
	public function action_controls( $widget, $step = false ){
		...
		$condition['main_action'] = 'duplicate_post';

		$widget->add_control(
			'post_to_duplicate',
			[
				'label' => __( 'Post To Duplicate', 'acf-frontend-form-element' ),
				'type' => Controls_Manager::SELECT,
		...
		$condition['main_action'] = ['edit_post', 'duplicate_post'];
	...
	public function on_submit( $post_id, $form ){	
		...
		foreach( $submit_keys as $key => $name ){
			...
			if( $name == 'post_status' && $post_to_edit[ $name ] == 'no_change' && $main_action == 'edit_post' ){
