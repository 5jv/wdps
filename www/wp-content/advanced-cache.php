...
// WP-Optimize advanced-cache.php (written by version: 3.1.8) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'F:\Wordpress local 2021\wdps\app\public\wp-content\plugins\wp-optimize\cache',
);
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', 'F:\Wordpress local 2021\wdps\app\public\wp-content\plugins\wp-optimize\cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-wdps.local.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-wdps.local.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-wdps.local.php');
}
